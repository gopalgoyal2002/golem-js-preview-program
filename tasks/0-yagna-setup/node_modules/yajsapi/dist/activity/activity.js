...
export class Activity {
    ...
    logger;
    isRunning = true;
    constructor(id, agreementId, options) {
        this.id = id;
        this.agreementId = agreementId;
        this.options = options;
        this.logger = options?.logger;
    }
    static async create(agreementId, options, secure = false) {
        const factory = new ActivityFactory(agreementId, options);
        return factory.create(secure);
    }
    async execute(script, stream, timeout) {
        ...
        this.options.eventTarget?.dispatchEvent(new Events.ScriptSent({ activityId: this.id, agreementId: this.agreementId }));
        return stream
    ...
    async pollingBatch(batchId, startTime, timeout) {
        ...
        return new Readable({
            ...
            async read() {
                while (!isBatchFinished) {
                    ...
                    try {
                        const { data: results } = await api.control.getExecBatchResults(activityId, batchId);
    ...
    async streamingBatch(batchId, batchSize, startTime, timeout) {
    ...
    async handleError(error, cmdIndex, retryCount, maxRetries) {
    ...
    parseEventToResult(msg, batchSize) {
        try {
