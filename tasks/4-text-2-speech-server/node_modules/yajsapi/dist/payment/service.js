...
export class PaymentService {
    ...
    logger;
    allocations = [];
    agreementsToPay = new Map();
    agreementsDebitNotes = new Set();
    invoicesToPay = new Map();
    debitNotesToPay = new Map();
    paidAgreements = new Set();
    payments;
    constructor(options) {
    ...
    async end() {
        ...
        for (const allocation of this.allocations)
            await allocation.release().catch((error) => this.logger?.warn(error));
    ...
    async createAllocations() {
        ...
        for (const account of accounts) {
            if (account.driver !== this.options.payment.driver.toLowerCase() ||
                account.network !== this.options.payment.network.toLowerCase()) {
                this.logger?.debug(`Not using payment platform ${account.platform}, platform's driver/network ` +
    ...
    acceptPayments(agreement) {
        this.agreementsToPay.set(agreement.id, agreement);
    }
    acceptDebitNotes(agreementId) {
    ...
    async processDebitNotes() {
        while (this.isRunning) {
            for (const debitNote of this.debitNotesToPay.values()) {
                if (!this.agreementsDebitNotes.has(debitNote.agreementId))
                    continue;
                try {
    ...
    getAllocationForPayment(paymentNote) {
        const allocation = this.allocations.find((allocation) => allocation.paymentPlatform === paymentNote.paymentPlatform && allocation.address === paymentNote.payerAddr);
        if (!allocation)
            throw new Error(`No allocation for ${paymentNote.paymentPlatform} ${paymentNote.payerAddr}`);
        return allocation;
    }
}
